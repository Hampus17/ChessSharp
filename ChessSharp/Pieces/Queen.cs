...
class Queen : Piece {
    ...
    public Queen(Color color, string type, int[] initalPos, char symbol) : base(color, type, initalPos, symbol) { }
    ...
    public override List<int[]> LegalMoves() {
        ...
        for (int i = 0; i < movesPerDir; i++) {
            moves.Add(new int[] { this.pos[0] - i, this.pos[1] }); // upwards
        }
        ...
        for (int i = 0; i < movesPerDir; i++) {
            moves.Add(new int[] { this.pos[0] + i, this.pos[1] }); // downards
        }
